...
def setup_aliases(monkeypatch):
    monkeypatch.setattr('docbot.settings.ALIASES', ','.join(TEST_ALIASES))


@pytest.fixture()
def dispatcher(monkeypatch):
    monkeypatch.setattr('docbot.settings.DEFAULT_REPLY', 'sorry')
    dispatcher = docbot.dispatcher.MessageDispatcher(None, None, None)
...
def test_dispatch_msg(dispatcher, monkeypatch):
    monkeypatch.setattr('docbot.dispatcher.Message', FakeMessage)
