...
"""
These functional tests would start a docbot, and use the slack web api to
...
from tests.functional.docbot_settings import (
...
def stop_proxy():
    os.system('docbot-test-ctl stopproxy')

def start_proxy():
    os.system('docbot-test-ctl startproxy')
...
def _start_bot_process():
    ...
    if TRAVIS:
        args = ['docbot-test-ctl', 'run'] + args
    env = dict(os.environ)
    env['docbot_API_TOKEN'] = testbot_apitoken
    env['docbot_TEST'] = 'true'
    env['PYTHONPATH'] = ':'.join(
